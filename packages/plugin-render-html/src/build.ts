...
const log = _log('@dev-spendesk/plugin-render-html')
    ...
    if (!htmls[resolved_fallback]) {
      if (typeof fallback === 'string') {
        throw new InvalidConfigError(
          `@dev-spendesk/plugin-render-html specifies a fallback of '${fallback}', which doesn't exist.`
        )
      } else {
        log.warn(
          `@dev-spendesk/plugin-render-html has 'fallback: true', but '${resolved_fallback}' doesn't exist! Skipping.`
        )
      }
    }
