...
const getLatestFabCli = async () => {
  const response = await fetch('https://registry.npmjs.org/@dev-spendesk/fab-cli/latest')
  ...
  if (data && data.version) {
    ...
    if (installed.version !== data.version) {
      log(
        `💚NOTE💚: You have 🖤@dev-spendesk/fab-cli🖤 💛${installed.version}💛, latest available on NPM is 💛${data.version}💛.`
      )
    }
  }
  return data
  ...
  static async init(
    ...
    if (empty) {
      ...
      if (this.yes) {
        ...
        • Install 💛@dev-spendesk/fab-cli💛 and related dependencies using 💛${
          use_yarn ? 'yarn' : 'npm'
        }💛`)
