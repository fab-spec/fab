...
import { MissingConfig, InvalidConfigError } from '@fab/core'
  ...
  static async readFrom(file_path: string): Promise<JSON5Config> {
    ...
    const readFile = async () => {
      try {
        ...
        throw new InvalidConfigError(`Could not read file at '${file_path}'`)
      }
    }
    const str_contents = await readFile()

    const parse = () => {
      try {
        return jju.parse(str_contents)
      } catch (e) {
