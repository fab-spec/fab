...
Note, you can use Typescript if you prefer, you only need to import the definition of `Runtime`

```ts
// proxy-api.ts
import { FABRuntime } from '@fab/core'

export default function ProxyApiHandler({ Router }: FABRuntime) {
  Router.on('/api/:route(.*)', ({ params }) =>
    fetch(`https://api.example.com/${params.route}`)
  )
}
