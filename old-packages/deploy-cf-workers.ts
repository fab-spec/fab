...
import CFDeployer from '../actions/CFDeployer'

export default class DeployCfWorkers extends Command {
  static description = 'Command line deployer for FABs to CF Workers (deprecated)'

  static examples = [`$ fab deploy-cf-workers fab.zip`]
  ...
  async run() {
    const { args, flags } = this.parse(DeployCfWorkers)
    const { file } = args
    ...
    await new CFDeployer().deploy(file, flags.config!, flags)
  }
}
