...
describe('Nextjs E2E Test', () => {
  ...
  it('should configure the NextJS project to produce FABs', async () => {
    ...
    // await shell(`pwd`, { cwd })
    // await shell(`yarn next build`, { cwd })
    // await shell(`yarn fab:build`, { cwd })
    const log = console.log.bind(console)
    await execa.command(`pwd`).then(log, log)
    await execa.command(`pwd`, { cwd }).then(log, log)
    await execa.command(`yarn build`, { cwd }).then(log, log)
    await execa.command(`yarn build:fab`, { cwd }).then(log, log)
